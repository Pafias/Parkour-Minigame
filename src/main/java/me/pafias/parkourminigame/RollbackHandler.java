...
public class RollbackHandler {
    ...
    public static void delete(String world) {
        for (Player player : Bukkit.getWorld(world).getPlayers())
            player.teleport(ParkourMinigame.getInstance().getLobby());
        String rootDirectory = ParkourMinigame.getInstance().getServer().getWorldContainer().getAbsolutePath();
        File worldFolder = new File(rootDirectory + "/" + world);
        Bukkit.unloadWorld(Bukkit.getWorld(world), false);
        deleteFolder(worldFolder);
        ParkourMinigame.getInstance().getServer().getScheduler().runTaskLater(ParkourMinigame.getInstance(), () -> Bukkit.createWorld(new WorldCreator(world.split("_")[0])), 60);
    }
